...
extension Peripheral.Environment {
  ...
  static func live(
    from cbPeripheral: CBPeripheral, subscriber: Effect<BluetoothManager.Action, Never>.Subscriber
  ) -> Self {
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
      subscriber.send(
        .peripheral(
          ...
          .didDiscoverServices(
            convertToResult(peripheral.services?.map(Service.init(from:)) ?? [], error: error))
        )
      )
    }
        ...
        .peripheral(
          ...
          .service(
            ...
            .didDiscoverIncludedServices(
              convertToResult(service.includedServices?.map(Service.init) ?? [], error: error))
          )
        )
      )
    }
        ...
        .peripheral(
          ...
          .service(
            ...
            .didDiscoverCharacteristics(
              convertToResult(
                service.characteristics?.compactMap(Characteristic.init) ?? [], error: error))
          )
        )
      )
    }
        ...
        .peripheral(
          ...
          .characteristic(
            ...
            .didDiscoverDescriptors(
              convertToResult(
                characteristic.descriptors?.compactMap(Descriptor.init) ?? [], error: error))
          )
        )
      )
    }
      ...
      subscriber.send(
        .peripheral(
          ...
          .descriptor(
            ...
            .didUpdateValue(
              convertToResult(
                Descriptor.anyToValue(uuid: descriptor.uuid, descriptor.value), error: error))
          )
        )
      )
    }
