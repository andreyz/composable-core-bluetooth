...
public struct PeripheralManager {
  ...
  var updateValue: (AnyHashable, Data, MutableCharacteristic, [Central]?) -> Effect<Bool, Never> = {
    _, _, _, _ in
    _unimplemented("updateValue")
  }

  var respondToRequest: (AnyHashable, ATTRequest, CBATTError.Code) -> Effect<Never, Never> = {
    _, _, _ in
    _unimplemented("respondToRequest")
  }

  var setDesiredConnectionLatency:
    (AnyHashable, CBPeripheralManagerConnectionLatency, Central) -> Effect<Never, Never> = {
      _, _, _ in
...
extension PeripheralManager {
  ...
  public func create(
    id: AnyHashable, queue: DispatchQueue?, options: InitializationOptions?
  ) -> Effect<Action, Never> {
  ...
  public func startAdvertising(
    id: AnyHashable, _ adverstimentData: AdvertismentData?
  ) -> Effect<Never, Never> {
  ...
  public func updateValue(
    id: AnyHashable, _ data: Data, for characteristic: MutableCharacteristic,
    onSubscribed centrals: [Central]?
  ) -> Effect<Bool, Never> {
    updateValue(id, data, characteristic, centrals)
  }

  public func respond(
    id: AnyHashable, to request: ATTRequest, with result: CBATTError.Code
  ) -> Effect<Never, Never> {
    respondToRequest(id, request, result)
  }

  public func setDesiredConnectionLatency(
    id: AnyHashable, _ latency: CBPeripheralManagerConnectionLatency, for central: Central
  ) -> Effect<Never, Never> {
...
extension PeripheralManager {
  ...
  public struct InitializationOptions {
    ...
    func toDictionary() -> [String: Any] {
      ...
      if let showPowerAlert = showPowerAlert {
        dictionary[CBPeripheralManagerOptionShowPowerAlertKey] = NSNumber(
  ...
  public struct RestorationOptions: Equatable {
    ...
    init(
      ...
      services = (dictionary[CBPeripheralManagerRestoredStateServicesKey] as? [CBService])?.map(
        Service.init)
      advertismentData =
        (dictionary[CBPeripheralManagerRestoredStateAdvertisementDataKey] as? [String: Any]).map(
