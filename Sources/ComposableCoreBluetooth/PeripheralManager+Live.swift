...
extension PeripheralManager {
  ...
  public static let live: PeripheralManager = { () -> PeripheralManager in
      ...
      .concatenate(
        ...
        Deferred(createPublisher: { () -> AnyPublisher<PeripheralManager.Action, Never> in
          dependencies[id]?
            ...
            .map(
              (/PeripheralManager.Action.didUpdateAdvertisingState
                .. Result<Bool, BluetoothError>.success).embed
            )
    ...
    manager.updateValue = { id, data, characteristic, centrals in
      .fireAndForget {
        dependencies[id]?.manager.updateValue(
          data, for: characteristic.cbMutableCharacteristic,
          onSubscribedCentrals: centrals?.compactMap(\.rawValue))
      }
    }
  ...
  class Delegate: NSObject, CBPeripheralManagerDelegate {
    ...
    init(
      _ subscriber: Effect<PeripheralManager.Action, Never>.Subscriber
    ) {
    ...
    func peripheralManager(
      ...
      guard let subscribedCharacteristic = Characteristic(from: characteristic) else { return }
      subscriber.send(.didSubscribeTo(subscribedCharacteristic, Central(from: central)))
    }
    ...
    func peripheralManager(
      ...
      guard let subscribedCharacteristic = Characteristic(from: characteristic) else { return }
      subscriber.send(
        .didUnsubscribeFrom(subscribedCharacteristic, Central(from: central)))
    }
    ...
    {
      if let request = ATTRequest(from: request) {
        subscriber.send(.didReceiveRead(request))
      }
    }
    ...
    func peripheralManager(
      ...
      subscriber.send(.didReceiveWrite(requests.compactMap(ATTRequest.init)))
    }
